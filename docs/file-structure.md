...
| Directory | Purpose |
| --- | --- |
| `src/ai-capabilities` | Authored integration layer owned by your team. Contains registries, runtime helpers, and capability definitions. |
| `src/ai-capabilities/capabilities` | Individual capability files created via `defineCapability` or `defineCapabilityFromExtracted`. |
| `src/ai-capabilities/auto` | Optional directory for conservative auto-bound capabilities generated by `npx ai-capabilities auto-bind`. Review/edit them and register the exports like any authored file. |
| `src/ai-capabilities/registry.ts` | Collects every capability and registers them with `registerCapabilityDefinitions`. |
...
Guidance:
- **Authored files (`src/ai-capabilities/**/*`)** live in source control and should be reviewed like any other application code.
- **Auto-bound files (`src/ai-capabilities/auto/**/*`)** are generated by the CLI to reduce toil. Treat them as a starting point—move/rename/merge them once you review the handler implementation.
